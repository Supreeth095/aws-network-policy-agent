...
func TestComputeTrieValueWithPolicy(t *testing.T) {
	port := int32(80)
	ports := []v1alpha1.Port{{Port: Ptr(port)}}

	result := ComputeTrieValueWithPolicy(ports, false, false, 1234, 200)
	require.Greater(t, len(result), 0)

	offset := 0
	protocol := binary.LittleEndian.Uint32(result[offset : offset+4])
	require.Equal(t, uint32(TCP_PROTOCOL_NUMBER), protocol)
	offset += 4

	start := binary.LittleEndian.Uint32(result[offset : offset+4])
	offset += 4
	end := binary.LittleEndian.Uint32(result[offset : offset+4])
	offset += 4
	require.Equal(t, uint32(port), start)
	require.Equal(t, uint32(port), end)

	policyID := binary.LittleEndian.Uint32(result[offset : offset+4])
	offset += 4
	require.Equal(t, uint32(1234), policyID)

	precedence := result[offset]
	require.Equal(t, uint8(200), precedence)
}
